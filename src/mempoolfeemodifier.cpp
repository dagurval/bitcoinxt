...
CAmount MempoolFeeModifier::GetDelta(const TxID& txid) const {
...
void MempoolFeeModifier::AddDelta(const TxID& txid, const CAmount& amount) {
    std::unique_lock<std::mutex> lock(cs);
...
void MempoolFeeModifier::RemoveDelta(const TxID& txid) {
    std::unique_lock<std::mutex> lock(cs);
    deltas.erase(txid);
}
