...
void ThinBlockWorker::buildStub(CNode& n, const StubData& d, const TxFinder& f) {
    assert(isWorkingOn(d.header().GetHash()));
    return mg.buildStub(*this, n, d, f);
}
...
bool ThinBlockWorker::addTxFirstBlock(const CTransaction& tx) {
    assert(blocks.size());
